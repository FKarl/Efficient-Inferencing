...
def main():
    ...
    parser.add_argument('--platon_no_mask', type=bool, default=False)
    ...
    generator = model.named_parameters()
    ...
    train_dataloader = load_data(args.dataset, 'train', args.task, args.batch_size, f'datasets/{args.dataset}/{tk.tokenizer.name_or_path}')
    validation_dataloader = load_data(args.dataset, 'validation', args.task, args.batch_size, f'datasets/{args.dataset}/{tk.tokenizer.name_or_path}')

    logging.info(f'Memory after dataloader: {memory_stats()}')
    
    # train
    logging.info('Pruning...')
    use_no_mask = True if args.model in ['DistilBERT', 'ALBERT'] else args.platon_no_mask

    model_handler = TransformerModel(model, training_loader=train_dataloader, validation_loader=validation_dataloader, load_optimizer=args.load_optimizer, model_dir=args.model_dir, task=args.task,  dataset=args.dataset, output_dir='pruned_models', model_name=args.optimized_model, optimized_model_name=args.optimized_model)
    model_handler.prune(epochs=args.epochs, learning_rate=args.learning_rate, weight_decay=args.weight_decay, num_warmup_steps=args.num_warmup_steps, use_no_mask=use_no_mask)

    logging.info('Saving model...')
    model_handler.save(prefix='pruned-')
