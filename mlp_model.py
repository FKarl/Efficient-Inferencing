...
def collate_for_mlp(list_of_samples):
    ...
    for doc, label in list_of_samples:
        ...
        labels.append(label)
        offset += len(doc)
    # torch.tensor(labels).dtype = int64
    return torch.tensor(flat_docs), torch.tensor(offsets), torch.tensor(labels)


def collate_for_mlp_multilabel(list_of_samples):
    """Collate function that creates batches of flat docs tensor and offsets"""
    offset = 0
    flat_docs, offsets, labels = [], [], []
    ...
    for doc, label in list_of_samples:
        ...
        # Convert the integer labels into binary tensors
    ...
    return torch.tensor(flat_docs), torch.tensor(offsets), torch.stack(labels, dim=0)
...
class MLP(nn.Module):
    ...
    def __init__(self, vocab_size, num_classes,
                 ...
                 embedding_dropout=0.5, multilabel=False):
        ...
        if multilabel:
            self.loss_function = nn.BCEWithLogitsLoss() # bce because each class is treated as a binary classification task
        else:
