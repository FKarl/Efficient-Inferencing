...
def train(args, train_dataset, model, tokenizer):
    ...
    train_dataloader = load_data(dataset_name="CiteWorth", split_name="test", task_type="CiteWorthiness", batch_size=64, dataset_path="datasets/CiteWorth/microsoft/deberta-base")
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=args.optimizer_warmup_steps, num_training_steps=len(train_dataloader)
    )
    ...
    logger.info( os.path.split(args.output_dir)[-1])
    ...
    PLATON = Pruner(model, args=args, total_step=len(train_dataloader), tb_writer=tb_writer, \
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    logger.info("[{No}] Epoch {epoch:d} | step: {global_step:d} | loss {loss:.5f} | threshold {threshold:.3f}".format(
                            No = No, epoch = epochs_trained, global_step = global_step, 
                            loss=(tr_loss - logging_loss) / args.logging_steps,
                            threshold = threshold, 
                        ))
                    logging_loss = tr_loss
                if args.local_rank == -1 and global_step % args.eval_steps == 0:
...
def main():
    ...
    parser.add_argument(
        "--output_root_folder",
        default="/home/v-minschen/qingru/DataLog/squad/adp_log",
        type=str,
        help="Where do you want to store the pre-trained models downloaded from huggingface.co",
    )
    ...
    parser.add_argument("--optimizer_warmup_steps", default=0, type=int, help="Linear warmup over warmup_steps.")
    parser.add_argument(
        "--n_best_size",
        default=20,
        type=int,
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        #model = .from_pretrained(args.output_dir)  # , force_download=True)
        model = AutoModelForSequenceClassification.from_pretrained(MODELS[args.model], num_labels=2,
                                                                problem_type="single_label_classification")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = AutoModelForSequenceClassification.from_pretrained(MODELS[args.model], num_labels=2,
                                                            problem_type="single_label_classification")  # , force_download=True)
            model.to(args.device)
