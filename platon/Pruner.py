...
class Pruner(object):
    def __init__(self, model, initial_threshold: float = 1, final_threshold: float = 0.10, initial_warmup: int = 1, final_warmup: int = 5, warmup_steps: int = 5400, total_step: int = 0, tb_writer=None, \
                beta1: float = 0.85, beta2: float = 0.95, deltaT: int = 10, \
    ...
    def schedule_threshold_comb(self, step: int):
        ...
        mask_ind = False
        if step <= self.initial_warmup * self.warmup_steps:
            threshold = self.initial_threshold
            mask_ind = False
        elif step > (total_step - self.final_warmup * self.warmup_steps):
            threshold = self.final_threshold
            mask_ind = True
        else: 
            spars_warmup_steps = self.initial_warmup*self.warmup_steps
            spars_schedu_steps = (self.final_warmup+self.initial_warmup)*self.warmup_steps
            mul_coeff = 1-(step-spars_warmup_steps)/(total_step-spars_schedu_steps)
            threshold = self.final_threshold+(self.initial_threshold-self.final_threshold)*(mul_coeff** 3)
    ...
    def mask_with_threshold(self, model, threshold):
        ...
        #print(is_dict)
        #print("Calculating mask threshold")
